      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500 font-medium uppercase tracking-widest mt-1">Manage stock, pricing, and categories</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-gray-200 border border-gray-200 rounded-sm overflow-hidden shadow-sm">
          <StatTile icon={<MdInventory />} label="Total Inventory" value={products.length + filterByCategories.length} color="text-gray-900" />
          <StatTile icon={<MdCheckCircle />} label="Available Units" value={products.filter(p => p.available).length} color="text-green-600" />
          <StatTile icon={<MdCancel />} label="Out of Stock" value={products.filter(p => !p.available).length} color="text-red-600" />
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-sm p-4 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-4">
          ...
          <div className="flex bg-gray-100 p-1 rounded-sm gap-1 w-full sm:w-auto overflow-x-auto">
            <FilterOption active={true} icon={<MdFilterList />} label="All" onClick={fetchProducts} />
            <FilterOption active={false} icon={<MdFilterList />} label="Available" />
            <FilterOption active={false} icon={<MdFilterList />} label="Low Stock" />
          </div>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-pulse">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-64 bg-gray-100 rounded-sm border border-gray-200" />
            ))}
          </div>
        ) : (
...
const StatTile = ({ icon, label, value, color }) => (
  <div className="bg-white p-5 flex items-center gap-4 group">
    <div className={`${color} bg-gray-50 p-3 rounded-sm group-hover:scale-110 transition-transform`}>
