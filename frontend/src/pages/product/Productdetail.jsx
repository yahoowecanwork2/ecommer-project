...
const Productdetail = () => {
  ...
  const groupedVariants = product
    ? Object.values(
        product.variants.reduce((acc, variant, index) => {
          const key = index % product.image.length;
          ...
          <div className="lg:col-span-6 space-y-8">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-baseline gap-4 pt-1">
                <span className="text-3xl font-light text-[#7A4431]">
                  ₹{discountedPrice}.00
                </span>
                {product?.discount > 0 && (
                  <span className="text-sm line-through text-gray-300 italic font-light">
                    ₹{finalPrice}
                  </span>
                )}
              </div>
            ...
            <div className="space-y-4 pt-4 border-t border-gray-50">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
                {product?.image.slice(0, 4).map((img, i) => (
                  ...
                  >
                    <div
                      className={`aspect-square w-16 overflow-hidden border transition-all ${activeImg === img.url ? "border-[#7A4431] ring-1 ring-[#7A4431]" : "border-gray-100 group-hover:border-[#7A4431]"}`}
                    >
                      <img
                        src={img.url}
                        className="w-full h-full object-cover"
                    ...
                    <p
                      className={`text-[8px] text-center font-bold uppercase py-1 ${activeImg === img.url ? "bg-[#7A4431] text-white" : "bg-gray-50 text-gray-400"}`}
                    >
                      Style {i + 1}
                    </p>
                  </div>
            ...
            <div className="flex flex-col gap-2 pt-4">
              ...
              <button
                ...
                className={`w-full py-4.5 flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] border transition-all ${
                  wish
                    ? "bg-[#7A4431] text-white border-[#7A4431]"
                    : "border-gray-200 text-[#2D1B2D] hover:bg-gray-50"
                }`}
              >
                {wish ? (
